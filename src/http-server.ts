...
async function main(): Promise<void> {
  console.log('Starting TestRail MCP HTTP server...');
  ...
  // Register all tools (same as original server.ts)
      ...
      req.on('end', async () => {
        try {
          ...
          if (request.method === 'notifications/initialized') {
            // N8n sends this after initialize - just acknowledge it
              ...
              { 
                ...
                inputSchema: {
                  ...
                  properties: {
                    ...
                    custom_step_results: {
                      type: 'array',
                      description: 'Array of step results for structured testing',
                      items: {
                        type: 'object',
                        properties: {
                          content: { type: 'string', description: 'The test step content' },
                          expected: { type: 'string', description: 'The expected result for the step' },
                          actual: { type: 'string', description: 'The actual result for the step' },
                          status_id: { type: 'integer', description: 'The status ID for the step (1=Passed, 2=Blocked, 4=Retest, 5=Failed)' }
                        },
                        required: ['content', 'expected', 'actual', 'status_id']
                      }
                    }
                  },
          ...
          if (request.method === 'tools/call') {
            ...
            // Route tool calls to appropriate service functions
            try {
              let result;
              
              switch (name) {
                case 'get_case':
                  result = await getCase(args.case_id);
                  break;
                case 'update_case':
                  result = await updateCase(args.case_id, args);
                  break;
                case 'get_projects':
                ...
                case 'get_project':
                  ...
                  break;
                case 'get_suites':
                  result = await getSuites(args.project_id);
                  break;
                case 'get_suite':
                  result = await getSuite(args.suite_id);
                  break;
                case 'get_cases':
                  result = await getCases(args);
                  break;
                case 'add_attachment_to_case':
                  result = await addAttachmentToCase(args.case_id, args.file_path);
                  break;
                case 'get_sections':
                  result = await getSections(args);
                  break;
                case 'get_runs':
                  result = await getRuns(args);
                  break;
                case 'get_run':
                  result = await getRun(args.run_id);
                  break;
                case 'update_run':
                  result = await updateRun(args.run_id, args);
                  break;
                case 'get_tests':
                  result = await getTests(args);
                  break;
                case 'get_test':
                  result = await getTest(args);
                  break;
                case 'update_test':
                  result = await updateTest(args.test_id, args);
                  break;
                case 'add_result':
                  result = await addResult(args);
                  break;
                default:
                  res.writeHead(404);
                  res.end(JSON.stringify({
          ...
          res.end(JSON.stringify({
            jsonrpc: '2.0',
            id: null,
            error: { code: -32700, message: 'Parse error', data: error?.message || 'Unknown error' }
          }));
        }
      });
      return;
  ...
  httpServer.listen(1823, () => {
    console.log('MCP HTTP server listening on port 1823');
  });
  
  console.log('MCP HTTP server ready on port 1823!');
}

main().catch((error: unknown) => {
  const err = error as Error;
  console.error('Failed to start MCP HTTP server:', err.message);
  console.error('Stack trace:', err.stack);
  process.exit(1);
});

process.on('exit', (code) => {
  console.log(`MCP HTTP server exiting with code: ${code}`);
});

process.on('SIGINT', () => {
  console.log('MCP HTTP server received SIGINT, shutting down...');
  process.exit(0);
});

process.on('SIGTERM', () => {
  console.log('MCP HTTP server received SIGTERM, shutting down...');
  process.exit(0);
});
